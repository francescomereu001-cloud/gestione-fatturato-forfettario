...
import { possibleTransfer } from "../services/bankImports";
...
export function TransactionsPage({ client, userId }: { client: SupabaseClient; userId: string }) {
  ...
  const linkTransfer = async (item: LedgerTransaction, match: LedgerTransaction) => {
    if (!item.id || !match.id) return;
    await client.rpc("link_internal_transfer", { first_transaction_id: item.id, second_transaction_id: match.id });
    await ledger.reload();
  };
  return <section className="panel"><h3>Transactions</h3><div className="cards mini"><div className="card"><p>Entrate</p><h2>{euro(summary.income)}</h2></div><div className="card"><p>Uscite economiche</p><h2>{euro(summary.expenses)}</h2></div><div className="card"><p>Cashflow</p><h2>{euro(summary.cashflow)}</h2></div></div>{ledger.error && <div className="notice">{ledger.error}</div>}
    ...
    <div className="ledgerTable">{shown.map(item => { const matches = item.transaction_type === "unclassified" ? possibleTransfer(item, ledger.transactions) : []; return <div className="ledgerRow transaction" key={item.id}><span>{item.transaction_date}</span><span><b>{item.description}</b><small>{ledger.accounts.find(a => a.id === item.account_id)?.name}</small>{matches.length === 1 && <small>Possibile giroconto: {ledger.accounts.find(a => a.id === matches[0].account_id)?.name}</small>}</span><span>{item.transaction_type}</span><span>{euro(Number(item.amount))}</span><span>{item.reconciliation_status}</span><span>{matches.length === 1 && <button className="ghost" onClick={() => void linkTransfer(item, matches[0])}>Collega</button>} {!item.transfer_group_id && <button className="ghost" onClick={() => setForm(item)}>Modifica</button>} <button className="iconBtn" onClick={() => remove(item)}><Trash2 size={16}/></button></span></div>; })}</div>
  </section>;
}
