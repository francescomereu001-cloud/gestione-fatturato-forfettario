...
import { possibleTransfer, transferTypeForAccounts } from "../services/bankImports";
...
const blankAccount = (): Account => ({ name: "", institution: "", account_type: "checking", currency: "EUR", opening_balance: 0, balance_as_of: null, is_active: true, include_in_liquidity: true, notes: "" });
...
export function AccountsPage({ client, userId }: { client: SupabaseClient; userId: string }) {
  ...
  return <section className="panel"><h3>Accounts</h3><p className="muted">Liquidità totale calcolata: <strong>{euro(totalLiquidity(ledger.accounts, ledger.transactions))}</strong></p>{ledger.error && <div className="notice">{ledger.error}</div>}
    <div className="formGrid compact"><label className="field">Nome<input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></label><label className="field">Istituto<input value={form.institution ?? ""} onChange={e => setForm({ ...form, institution: e.target.value })} /></label><label className="field">Tipo<select value={form.account_type} onChange={e => { const account_type = e.target.value as Account["account_type"]; setForm({ ...form, account_type, include_in_liquidity: account_type === "credit_card" ? false : form.include_in_liquidity }); }}>{accountTypes.map(type => <option key={type}>{type}</option>)}</select></label><label className="field">Saldo all’anchor<input type="number" value={form.opening_balance} onChange={e => setForm({ ...form, opening_balance: Number(e.target.value) })} /></label><label className="field">Saldo alla chiusura del<input type="date" value={form.balance_as_of ?? ""} onChange={e => setForm({ ...form, balance_as_of: e.target.value || null })} /></label><label className="check"><input type="checkbox" checked={form.include_in_liquidity} onChange={e => setForm({ ...form, include_in_liquidity: e.target.checked })} /> Include in liquidità</label></div>
    <button className="primary" onClick={save}><Save size={18} />{form.id ? "Aggiorna" : "Crea conto"}</button>
    <div className="ledgerTable mt">{ledger.accounts.map(account => <div className="ledgerRow" key={account.id}><span><b>{account.name}</b><small>{account.institution || "—"}</small></span><span>{account.account_type}</span><span>{euro(accountBalance(account, ledger.transactions))}<small>{account.balance_as_of ? `Anchor: ${account.balance_as_of}` : "⚠ Data saldo mancante"}</small></span><span>{account.include_in_liquidity ? "Inclusa" : "Esclusa"}</span><span><button className="ghost" onClick={() => setForm(account)}>Modifica</button> {account.is_active && <button className="iconBtn" title="Disattiva" onClick={() => deactivate(account)}><Trash2 size={16}/></button>}</span></div>)}</div>
  </section>;
}
...
export function TransactionsPage({ client, userId }: { client: SupabaseClient; userId: string }) {
  ...
  const linkTransfer = async (item: LedgerTransaction, match: LedgerTransaction, targetType: "internal_transfer" | "investment_transfer") => {
    if (!item.id || !match.id) return;
    await client.rpc("link_transfer", { first_transaction_id: item.id, second_transaction_id: match.id, target_type: targetType });
    await ledger.reload();
  };
  return <section className="panel"><h3>Transactions</h3><div className="cards mini"><div className="card"><p>Entrate</p><h2>{euro(summary.income)}</h2></div><div className="card"><p>Uscite economiche</p><h2>{euro(summary.expenses)}</h2></div><div className="card"><p>Cashflow</p><h2>{euro(summary.cashflow)}</h2></div></div>{ledger.error && <div className="notice">{ledger.error}</div>}
    ...
    <div className="ledgerTable">{shown.map(item => { const matches = item.transaction_type === "unclassified" ? possibleTransfer(item, ledger.transactions) : []; const match = matches.length === 1 ? matches[0] : null; const sourceType = ledger.accounts.find(account => account.id === item.account_id)?.account_type; const destinationType = match ? ledger.accounts.find(account => account.id === match.account_id)?.account_type : undefined; const targetType = sourceType && destinationType ? transferTypeForAccounts(sourceType, destinationType) : "internal_transfer"; return <div className="ledgerRow transaction" key={item.id}><span>{item.transaction_date}</span><span><b>{item.description}</b><small>{ledger.accounts.find(a => a.id === item.account_id)?.name}</small>{match && <small>Possibile trasferimento: {ledger.accounts.find(a => a.id === match.account_id)?.name}</small>}</span><span>{item.transaction_type}</span><span>{euro(Number(item.amount))}</span><span>{item.reconciliation_status}</span><span>{match && <button className="ghost" onClick={() => void linkTransfer(item, match, targetType)}>{targetType === "investment_transfer" ? "Conferma investimento" : "Conferma giroconto"}</button>} {!item.transfer_group_id && <button className="ghost" onClick={() => setForm(item)}>Modifica</button>} <button className="iconBtn" onClick={() => remove(item)}><Trash2 size={16}/></button></span></div>; })}</div>
  </section>;
}
